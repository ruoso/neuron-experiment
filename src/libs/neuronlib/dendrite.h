...
inline bool is_terminal_address(uint32_t address) {
    return (address & TERMINAL_MASK) != 0;
}

inline bool is_branch_address(uint32_t address) {
    return (address & TERMINAL_MASK) == 0 && (address & DENDRITE_MASK) != 0;
}

inline bool is_neuron_address(uint32_t address) {
    return (address & DENDRITE_MASK) == 0;
}

inline uint32_t get_terminal_branch(uint32_t terminal_address) {
    return terminal_address & ~TERMINAL_MASK;
}

inline uint32_t get_parent_branch(uint32_t branch_address) {
...
inline uint32_t get_neuron_address(uint32_t dendrite_address) {
